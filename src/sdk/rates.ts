...
export class Rates {
     ...
     */
    async getRates(
        req: shared.RateRequest,
        config?: AxiosRequestConfig
    ): Promise<operations.GetRatesResponse> {
        if (!(req instanceof utils.SpeakeasyBase)) {
            req = new shared.RateRequest(req);
        }
        ...
        const url: string = baseURL.replace(/\/$/, "") + "/Rates";
        ...
        switch (true) {
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.ratesResponse = utils.objectToClass(
                        JSON.parse(decodedRes),
                        shared.RatesResponse
                    );
                } else {
                ...
                break;
            case [401, 422, 500].includes(httpRes?.status):
                if (utils.matchContentType(contentType, `application/json`)) {
